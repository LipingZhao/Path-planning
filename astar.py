...
map_grid = numpy.full((10, 10), int(10), dtype=numpy.int8)
map_grid[2, 2:4] = 0
map_grid[2, 6:8] = 0
map_grid[3, 2:4] = 0
map_grid[3, 6:8] = 0
...
class AStar(object):
    ...
    def __init__(self):
        ...
        """
        # self.g = 0  # g初始化为0
        self.start = numpy.array([0, 0])  # 起点坐标
        self.goal = numpy.array([9, 9])  # 终点坐标
    ...
    def child_point(self, x):
        ...
        for j in range(-1, 2, 1):
            for q in range(-1, 2, 1):
                ...
                m = [x[0]  + j, x[1] + q]
                print(m)
                if m[0] < 0 or m[0] > 9 or m[1] < 0 or m[1] > 9:  # 搜索点出了边界去掉
                    continue
    ...
    def path_backtrace(self):
        ...
        :return:
        """
        best_path = [9, 9]  # 回溯路径的初始化
        self.best_path_array = numpy.array([[9], [9]])
        j = 0
    ...
    def main(self):
        ...
        while ite <= 200:
                ...
                print("numpy.lexsort(self.open):",numpy.lexsort(self.open))
                ...
                if best[0] == 9 and best[1] == 9:  # 如果best是目标点，退出
                    print('搜索成功！')
                    return
...
class MAP(object):
    ...
    def draw_init_map(self):
        ...
        xlim(-1, 10)  # 设置x轴范围
        ylim(-1, 10)  # 设置y轴范围
        my_x_ticks = numpy.arange(0, 10, 1)
        my_y_ticks = numpy.arange(0, 10, 1)
